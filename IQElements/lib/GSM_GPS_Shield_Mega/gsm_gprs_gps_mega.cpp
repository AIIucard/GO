...
int GSM_GPRS_Class::initialize(const char simpin[4])
{
  ...
  do
  {
    ...
    if(state == 3)
    {
      _HardSerial.print("AT#QSS=0\r");                                               // Query SIM Status (disables the unsolicited indication)
      if(WaitOfReaction(1000) == 1) { state += 1; } else { state = 1000; }
    }
    ...
    if(state == 5)
    {
      ...
      switch (WaitOfReaction(1000))
      {
        ...
        default: _HardSerial.println("Serious SIM-error!"); while(1);				  // ATTENTION: check your SIM!!!! Don't restart the software!!!
      }
    }
...
int GSM_GPRS_Class::Status()
{
  ...
  do
  {
    ...
    if(state == 1)
    {
      _HardSerial.print("AT+CGREG?\r");                                              // Query register state of GPRS network
      WaitOfReaction(1000);
...
int GSM_GPRS_Class::deleteSMS(int index)
{
  ...
  do
  {
    ...
    if(state == 2)
    {
      if (index == 0)
      {
        _HardSerial.print("AT+CMGD=1,4\r");                                          // Ignore the value of index and delete *all* SMS messages
      }
      else
      {
        _HardSerial.print("AT+CMGD=");                                               // delete the SMS with the given index (page 84)
        _HardSerial.print(index);														    		  // index range = 1 ... x
        _HardSerial.print("\r");
      }
      if(WaitOfReaction(1000) == 1)
...
The public variable "GSM_string" contains the last response from the mobile module
*/
int GSM_GPRS_Class::connectGPRS(const char APN[50],const char USER[30],const char PWD[50])
{
  int time = 0;
